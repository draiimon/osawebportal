...
function AnnouncementCard({ item }) {
  return (
    <article className="border border-red-200 bg-white p-4">
      ...
      <p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-700">{item.details || "No details provided."}</p>
      <p className="mt-3 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
        {item.date || "Date TBD"} {item.time ? `• ${item.time}` : ""}
      </p>
    </article>
  );
}
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-4 py-6">
      ...
      {!loading && !error && items.length === 0 ? (
        <p className="border border-red-200 bg-white px-4 py-3 text-sm text-slate-700">No announcements found.</p>
      ) : null}
