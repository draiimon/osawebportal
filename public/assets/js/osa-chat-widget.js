...
(function () {
    ...
    function buildMarkup() {
        return '' +
            ...
            '      </div>' +
            '      <button type="button" class="osa-ai-header-signout" id="osa-chat-end-session" hidden aria-label="Sign out" title="End secure session">' +
            '        <svg class="osa-ai-header-signout__icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>' +
            '        <span class="osa-ai-header-signout__label">Sign out</span>' +
            '      </button>' +
    ...
    function init() {
        ...
        function updateQuotaPill(quota) {
            ...
            if (!pill || !text) return;
            // OTP-verified students get unlimited daily messages; hide the pill
            // entirely so the composer stays clean.
            if (quota.unlimited === true) {
                quotaState = { used: 0, limit: null, remaining: null, fetched: true, unlimited: true };
            ...
            quotaState = { used: used, limit: limit, remaining: remaining, fetched: true, unlimited: false };
            text.textContent = used + ' / ' + limit + ' today';
            pill.removeAttribute('hidden');
            pill.classList.remove('is-low', 'is-empty');
            if (remaining === 0) {
                pill.classList.add('is-empty');
                text.textContent = 'Daily limit reached — verify with OTP for unlimited';
            } else if (remaining <= 3) {
